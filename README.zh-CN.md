...
#### 1. 基于核心架构，确保底座安全
从零开始让 AI 搭建框架，难以保证架构质量与安全性。因此，**强烈建议让 AI 在已验证的核心基础框架上进行开发**。
- **优势**：框架底座可靠、安全可控，AI 仅需专注于业务逻辑的实现，大幅提升开发效率与代码质量。

#### 2. 前后端配置 `AGENTS.md`，统一上下文认知
`AGENTS.md` 为 AI 的专属上下文指南。 让 AI 清晰了解前后端的核心架构、技术栈及接口对接规范，避免“幻觉”与架构偏离。

#### 3. 单 Prompt 驱动完整业务生成
经实测，仅需 **一个 Prompt** 即可让 AI 生成完整且可运行的前后端业务。
- **示例场景**：“*有一个 XX 业务，包含 Admin 和 User 角色，分别有 XX 功能。*” (实际的 Prompt 复杂很多)
- **执行效果**：AI 能够精准理解已设计好的 RBAC 权限架构，自动完成业务表结构设计及前后端代码生成，实现端到端的业务落地。
